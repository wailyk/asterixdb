...
public class PushFieldAccessToDataScan implements IAlgebraicRewriteRule {
    ...
    private void setDataset(ILogicalOperator op, MetadataProvider mp) throws AlgebricksException {
        ...
        if (dataset == null
                || dataset.getDatasetType() != DatasetType.INTERNAL && dataset.getDatasetType() == DatasetType.EXTERNAL
                        && !((ExternalDatasetDetails) dataset.getDatasetDetails()).allowsProjectPushdown()) {
            return;
        }
        ...
        scanOps.add(scan);
        scan.setAllowsPush(dataset.getDatasetType() == DatasetType.INTERNAL);
    }
